$(document).ready(function () {
    ...
    $("button.btn_reg").click(function () {
        ...
        if (check_fullname() && check_number_phone() && check_address() && check_check_address() && check_proof_document() && check_list_exhibit() && check_number_loan()) {
            ...
            var number_phone = $("#number_phone").val();
            var address = $("#address").val();
            let check_address = $("input[name=check_address]:checked").val();
            if (check_address == 'yes') {
                var proof_document = "Địa chỉ đang ở trùng hộ khẩu";
            }
            if (check_address == 'no') {
            ...
            $.ajax({
                type: "post",
                url: "?mod=page&action=send_info_customer",
                data: data,
                dataType: "text",
                success: function (result) {
                    alert(result);
...
function check_number_phone() {
    ...
    if (number_phone.length == 0) {
        ...
        return false;
    } else if (!validnumber_phone || number_phone != 10) {
...
function check_number_loan() {
    var number_loan = $("#number_loan").val();
    if (number_loan == "" || number_loan == 0) {
        $("#error_number_loan").html("*Vui lòng nhập số tiền vay");
        return false;
