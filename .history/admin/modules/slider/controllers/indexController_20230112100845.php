    ...
    if (isset($_POST['add_slider_mobile'])) {
        ...
        if (!in_array(strtolower($type), $type_allow)) {
            ...
            if ($file_size < 21000000) {
                ...
                $upload_file_mobile = $file_dir . $file_name . '.' . $type;
                if (!file_exists($upload_file_mobile)) {
                    if (move_uploaded_file($_FILES['file_mobile']['tmp_name'], $upload_file_mobile)) {
        ...
        if (empty($error)) {
            if (file_exists($upload_file_mobile)) {
                $upload_file_mobile = $file_dir . $file_name . ' - Copy' . '.' . $type;
            }
            $i = 2;
            while (file_exists($upload_file_mobile)) {
                $upload_file_mobile = $file_dir . $file_name . ' - Copy(' . $i . ')' . '.' . $type;
                $i++;
            }
            if (move_uploaded_file($_FILES['file_mobile']['tmp_name'], $upload_file_mobile)) {
                $success_mobile['upload'] = "Upload file thành công";
