...
function add_img_homeAction()
{
    ...
    if (isset($_POST['change_img_home'])) {
        ...
        if (empty($error)) {
            ...
            if (move_uploaded_file($_FILES['file']['tmp_name'], $upload_file)) {
                ...
                $where = "`id` = 0";
                $db = new DB;
                $field = array('path_admin');
                $path_admin_old = $db->get($table, $field, "`id` = 0");
                $result_unlink = unlink($path_admin_old);
                if ($db->update($table, $data, $where) && $result_unlink) {
